...
- 空调、IC卡和监控额外支持老价格、新价格、Saving、CHECK 的批量计算；FLD 项目按 PO 分组合并显示。
...
3. 按工作台中的步骤执行。通常顺序为：生成透视表 → 添加扩展列 → 查询下载 → 回填 → PM 匹配；空调、IC卡或监控最后再执行价格填充。
